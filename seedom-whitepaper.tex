...
\hline
\textbf{Parameter} & \textbf{Data Type} & \textbf{Description} \\ \hline
maxScore & uint256 & the max score of a vote \\ \hline
fundraiser & address & link to the fundraiser contract \\ \hline
\end{tabular}
...
Because Ethereum is a Turing complete deterministic world computer, mining nodes cannot generate global random messages as they would never be able to reach consensus. Many Ethereum pseudo-random number generation methods exist, with each having various pros and cons \cite{12}.

\begin{table}[H]
\begin{center}
\begin{tabular}{| l | l | l | l | l |}
\hline
\textbf{Method} & \textbf{On-chain} & \textbf{One Transaction} & \textbf{Safe Against Miners} & \textbf{No Time Constraint} \\ \hline
Block variables & X & X & & X \\ \hline
Current blockhash & X & X & & X \\ \hline
Last blockhash & X & X & & X \\ \hline
Future blockhash & X & & X & \\ \hline
RANDAO (Commit-reveal) & X & & X & X \\ \hline
External oracle & & X & X & X \\ \hline
Seedom & X & X & X & X \\ \hline
\end{tabular}
\caption{Pseudo-random number generation methods}
\label{tab:pseudoRandomNumberGenerationMethods}
\end{center}
\end{table}

The usage of block variables as a source of entopy is usually flawed given a miner's ability to manipulate all of these values. While the future blockhash method is secure, it requires a user to issue multiple transactions in order to generate a random number. The same is true of the RANDAO with the commit and reveal phases. Both of these methods introduce extra steps that decrease the ease of use of the application.\\\\
Extra work must be done in the RANDAO method to prevent miner manipulation as miners can ignore transactions, reorder them, and avoid broadcasting blocks entirely. It opens up the potential for the miner to receive additional chances of winning, even with deincentivization enhancements. Other methods of random message generation include the usage of an external oracle, which requires a participant to trust the availability and validity of such service.\\\\
The Seedom method involves receiving two secrets, or hashed messages, guaranteed to be from Seedom and the cause. Once these secrets store on-chain, the collection of public messages from participants can occur during the participation and voting phase. After the fundraiser end time, Seedom and the cause reveal their secrets, which store on-chain after they are verified, using the formula in Figure \ref{figure:secretGenerationFormula}.\\\\
These two revealed messages are XORed together to generate something call the administrator random message, which is then used to select a single participant's message. The administrator random message is then XORed with the randomly selected participant's message to produce a final crowd-sourced random message. This final random message is used to select yet another participant to actually reward, a process displayed visually in Figure \ref{figure:crowdsourcedRandomMessageGeneration}.
...
Ethereum miners have much flexibility is regards to Seedom relies on a randomization effort from the Seedom team, the cause, and a randomly selected participant.
